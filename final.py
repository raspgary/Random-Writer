...
class RandomWriter(object):
    ...
    def __init__(self, level, tokenization=None):
        ...
        if self.tokenization is None:
            self.tokenization = Tokenization.none
        self.level = level
    ...
    def generate(self):
        ...
        state = random.choice(list(self.g.vertices))
        state = self.g.vertices[state]
        while True:
            while state.get_total_scratch() == 0:
                state = random.choice(list(self.g.vertices))
                state = self.g.vertices[state]
            random_val = random.randint(0, state.get_total_scratch() - 1)
            token = None
            # choose random edge
            for e in state.edges:
                if random_val < state.edges[e]:
                    state = e.destination
                    token = e.value
                    break
                else:
                    random_val -= state.edges[e]
            yield token
            ...
            with open(filename, mode="w", encoding="utf-8") as fi:
                for token in itertools.islice(self.generate(), amount):
                    if self.tokenization == Tokenization.none \
                            or self.tokenization == Tokenization.word:
                        fi.write(str(token))
                        fi.write(" ")
                    else:
                        fi.write(token)
    ...
    def save_pickle(self, filename_or_file_object):
        ...
        if isinstance(filename_or_file_object, str):
            with open(filename_or_file_object, mode="wb") as fi:
                pickle.dump(self, fi)
        else:
    ...
    def load_pickle(cls, filename_or_file_object):
        ...
        """
        if isinstance(filename_or_file_object, str):
            with open(filename_or_file_object, mode="rb") as fi:
                p = pickle.load(fi)
        else:
            p = pickle.load(filename_or_file_object)
        rw = RandomWriter(p.level, p.tokenization)
        return rw
    ...
    def train_url(self, url):
        ...
        """
        if self.tokenization is not Tokenization.none:
            ...
            raise TypeError("Not proper tokenization")
    ...
    def train_iterable(self, data):
        ...
        # from final_tests written by Arthur Peters
        ...
        if self.tokenization == Tokenization.word and isinstance(data, str):
            data = data.split()
        elif self.tokenization == Tokenization.none and hasattr(data, '__iter__')\
                or self.tokenization == Tokenization.character and isinstance(data, str)\
                or self.tokenization == Tokenization.byte and isinstance(data, bytes):
            data = data
        else:
            raise TypeError("Not proper tokenization type")
        history = None
        for i in windowed(data, self.level):
            self.g.add_vertex(i)
            if history is not None:
                self.g.add_edge(i[-1], history, i)
            history = i
